...
设备配置名来自 `wrt_core/compilecfg/` 和 `wrt_core/deconfig/` 中同名文件。当前支持以下配置，但 ⭐ 标记的为实际编译设备，其余为上游兼容保留：
...
Link NN6000v2 前面板有一颗 RGB 三色状态指示灯（红/绿/蓝），由 GPIO 50/70/69 通过 `leds-gpio` 驱动。

#### GPIO 极性修正

ImmortalWRT 上游 DTS 中 GPIO flags 为 `GPIO_ACTIVE_HIGH`(0x00)，但硬件为低电平有效，导致 `brightness` 值反相（写 1 灭、写 0 亮）。
...
固件内置 `led-ctrl` 服务，将 RGB 灯珠作为整体管理，每次只亮所需颜色避免非预期混色，实现 5 种互联网状态指示：

| 灯光 | 状态 | 含义 |
|------|------|------|
| 🔴 红常亮 | `no-link` | WAN 接口 down（网线未插） |
| 🟡 黄快闪 | `dialing` | PPPoE 拨号中 |
| 🟡 黄常亮 | `no-inet` | WAN up 但 ping 不通 |
