...
update_lucky() {
    ...
    local version
    version=$(find "$BASE_PATH/prebuilt_packages" -name "lucky_*.tar.gz" -printf "%f\n" | head -n 1 | sed -n 's/^lucky_\(.*\)_Linux.*$/\1/p')
    if [ -z "$version" ]; then
        echo "Warning: 未找到 lucky 预编译包文件，跳过更新。" >&2
        return 0
    fi
    ...
    local patch_line="\\t[ -f \$(TOPDIR)/../wrt_core/prebuilt_packages/lucky_${version}_Linux_\$(LUCKY_ARCH)_wanji.tar.gz ] && install -Dm644 \$(TOPDIR)/../wrt_core/prebuilt_packages/lucky_${version}_Linux_\$(LUCKY_ARCH)_wanji.tar.gz \$(PKG_BUILD_DIR)/\$(PKG_NAME)_\$(PKG_VERSION)_Linux_\$(LUCKY_ARCH).tar.gz"
