...
-- ⭐ 本地定制：值改为 openvpn 服务端形式（netifd ovpnproto 直接消费，客户端 ovpn 由 genovpn.sh 转换）
o:value("tcp-server",translate("TCP"))
o:value("udp",translate("UDP"))
o:value("tcp6-server",translate("TCP Over IPv6"))
...
-- ⭐ 本地定制：保存后自动应用（ImmortalWRT 25.12 openvpn 无官方 init.d，
-- 由本地 /etc/init.d/openvpn 桥接脚本同步到 netifd 接口并按 enabled 启停）
