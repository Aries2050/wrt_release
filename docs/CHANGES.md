...
| 2026-09-03 | `2a71290` | 新增 QModem（5G/4G Modem 管理）集成：`feeds.sh` 添加 `src-git qmodem`（FUjr/QModem，多包分层 feed）；亚瑟 `jdcloud_ipq60xx_immwrt.config` 启用 `qmodem` + `luci-app-qmodem-next`（新版 JS UI，自带短信）+ `sms-forwarder-next` + `qmodem-seal` + `quectel-CM-5G-M`，补充通用 USB modem 内核模块（`kmod-usb-net-cdc-mbim` / `kmod-usb-net-qmi-wwan` / `kmod-usb-serial-option`）；驱动选型 = 通用 QMI 编入固件、厂商驱动（`kmod-qmi_wwan_q/s/f`、`kmod-pcie_mhi`）仅编译产出 APK 不内置（经 `firmware/packages/` 本地源按需安装）；详见 [qmodem-integration.md](./qmodem-integration.md) |
| 2026-09-04 | `28d7512` | 修复 CI 构建失败（qca-nss-ecm modpost）：上游 qca-nss-ecm 以 `CONFIG_PACKAGE_kmod-qmi_wwan_q` 为 RAWIP 前端开关（`ifneq` 对 `=y`/`=m` 均成立），`=m` 也会触发 ECM RAWIP 编译、需要 NSS rmnet 符号 `nss_rmnet_rx_get_ifnum` 而默认未导出 → `ERROR: modpost: "nss_rmnet_rx_get_ifnum" [ecm.ko] undefined!`；修复 = 厂商 Quectel QMI 驱动 `kmod-qmi_wwan_q` 改为 `=y` 内置 + `fragments/nss.config` 启用 `CONFIG_NSS_DRV_RMNET_ENABLE=y` 导出符号（Simcom/Fibocom `kmod-qmi_wwan_s/f`、`kmod-pcie_mhi` 保持 `=m` 仅编译）；另修 `build.sh` 失败后自动补跑冲突检测的路径 bug（`$BASE_PATH/scripts` → `$REPO_ROOT/scripts`，原指向不存在的 `wrt_core/scripts/`）；⚠️ 该 rmnet 方案后证不完整：`nss_rmnet_rx_get_ifnum` 由 QModem `rmnet-nss` 提供（仅 ipq807x/ipq50xx），ipq60xx 无提供者，最终以同日二修禁用 ECM RAWIP 联动解决；详见 [qmodem-integration.md](./qmodem-integration.md) |
| 2026-09-04 | `当前提交` | 修复 CI 构建失败（qca-nss-ecm modpost，二修·最终）：ipq60xx 无 NSS rmnet 提供者（QModem `rmnet-nss` 约束 `@(TARGET_qualcommax_ipq807x||ipq50xx)`，不含 ipq60xx），`CONFIG_NSS_DRV_RMNET_ENABLE=y` 无法提供 `nss_rmnet_rx_get_ifnum` → 改为构建期禁用 ECM RAWIP 联动：新增 `package_source_updates.sh` 的 `disable_qca_nss_ecm_rawip()`（`update.sh` stage_pre_install_source_fixes 注册）删除 `qca-nss-ecm/Makefile` 的 `kmod-qmi_wwan_q`→RAWIP 整块；移除 `fragments/nss.config` 的无效 `CONFIG_NSS_DRV_RMNET_ENABLE=y`；`kmod-qmi_wwan_q` 保持 `=y` 内置（走标准 QMI/AT），`kmod-qmi_wwan_s/f`、`kmod-pcie_mhi` 保持 `=m` 仅编译；详见 [qmodem-integration.md](./qmodem-integration.md) |
...
| `qmodem` + `luci-app-qmodem-next`（QModem 套件） | `2a71290`（接入）；`当前提交`（09-04 修复：禁用 ECM RAWIP 联动） | 5G/4G Modem 管理（FUjr/QModem feed，`feeds.sh` 添加 `src-git qmodem`）；亚瑟 `jdcloud_ipq60xx_immwrt.config` 启用核心 + 新版 JS UI（自带短信）+ `sms-forwarder-next` + `qmodem-seal` + `quectel-CM-5G-M`；通用 USB modem 内核模块（cdc-mbim/qmi-wwan/serial-option）编入固件；厂商驱动：Quectel `kmod-qmi_wwan_q` 内置（构建期禁用 ECM RAWIP 联动因 ipq60xx 无 rmnet 提供者），Simcom/Fibocom `kmod-qmi_wwan_s/f` / `kmod-pcie_mhi` 仅编译不内置；详见 [qmodem-integration.md](./qmodem-integration.md) |
