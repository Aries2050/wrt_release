...
| 2026-08-16 | `当前提交` | 停用 easymesh：CI 构建在 apk install 阶段报 `unable to select packages: batctl-default / kmod-batman-adv`——`luci-app-easymesh`（kenzok8 版，a02cc31 引入）依赖的 `kmod-batman-adv`、`batctl-default` 在 ImmortalWRT 25.12 源码树中**不存在**（构建日志 `No feed for package ... found`，内核默认 `CONFIG_BATMAN_ADV` 未启用），之前的构建均死在更早的 qca-ssdk/openvpn-easy-rsa，该依赖问题从未暴露；`compile_base.config` 注释 `luci-app-easymesh` 与 `luci-i18n-easymesh-zh-cn`，待上游恢复 batman-adv 支持后再启用 |
